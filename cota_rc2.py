...
def batch_bytes_to_vector(items, device=None):
    ...
    Uses the same Vandermonde polynomial embedding as bytes_to_vector —
    results are geometrically compatible for distance comparisons.
    """
    if device is None:
        device = DEVICE
    vecs = []
    for data in items:
        vecs.append(bytes_to_vector(data, device=device))
    if not vecs:
...
class HyperbolicBuffer:
    ...
    def assimilate(self, soul, max_items=5):
        ...
        for k, item, dist in candidates[:max_items]:
            ...
            if accepted:
                # LI written by evolve_sequence — source is process label, not content
                pass
...
class Soul:
    ...
    def evolve(self, input_vec, source="user"):
        ...
        # Internal source sets — available to both accept and reject paths
        ...
        if delta_e <= margin:
            ...
            if not _is_internal:
                ...
                # NOTE: LI is written by evolve_sequence via _commit_fragment,
                # not here. source is a process label, not content.
            self.accepted += 1
            ...
            # Vocabulary exists even if rejected — register in LI if source is real content
            # (bytes not in internal set = came from actual input, not process machinery)
            if (isinstance(source, (bytes, bytearray))
                    and source not in _INTERNAL_BYTES
                    and source.strip()):
                _rej_text = source.rstrip(b'\x00').decode('utf-8', errors='replace').strip()
                if _rej_text:
                    self.language_index.add(candidate, _rej_text)
            self.rejected += 1
    ...
    def abstract_memory(self, merge_radius=0.15, samples=200):
        ...
        for _ in range(samples):
            ...
            if actual_dist < merge_radius:
                ...
                # Centroid has no label — it will acquire one when
                # a future input byte string lands geometrically near it
                self.attractor_lexicon.rg_merge(vec, vec2, centroid, "")
                if hasattr(self, 'rg_analytics'):
                    self.rg_analytics.log_merge(d_a, d_b, d_c, delta_e, self.tau, accepted=True)
                merged += 1
                self.history.append(self.state.clone())
            ...
            if k % _atom_every == 0:
                ...
                if verbose:
                    print(f"  [RG] fusion={_fusion_norm:.3f} "
                          ...
                          f"ratio={_ratio:.3f} [{_regime}] (atomized={na})")
                if diagnostics is not None:
...
class NarrativeGenerator:
    ...
    # Tokens too short to be meaningful output
    _MIN_TOKEN_LEN = 2
    # Punctuation/symbols that should not appear as standalone tokens
    _SKIP_TOKENS = set('|\\/:;,.!?@#$%^&*()[]{}\'"`~')

    def _get_candidates(self, state):
        li = getattr(self.soul, 'language_index', None)
        if li is not None and li.size() > 0:
            # Fetch many more than needed — single-byte tokens dominate
            # the nearest-neighbour results and most get filtered
            texts = li.nearest(state, k=self.K_CANDIDATES * 10)
            if texts:
                seen = set()
                cands = []
                for txt in texts:
                    ...
                    seen.add(txt)
                    # Skip single chars that are punctuation/symbols
                    if len(txt) < self._MIN_TOKEN_LEN and txt in self._SKIP_TOKENS:
                        continue
                    # Skip non-printable tokens (hex escapes, control chars)
                    if not all(32 <= ord(ch) < 127 or ch.isalpha() for ch in txt):
                        continue
                ...
                if cands:
                    return cands
        # Fallback: rg_output vocabulary (has word-length tokens)
        rgo = getattr(self.soul, 'rg_output', None)
        if rgo is not None and rgo.vocab_size() > 0:
            cands = []
            tokens, vecs = rgo._tokens, rgo._vectors
            n = min(len(tokens), len(vecs))
            if n > 0:
                stack  = torch.stack(vecs[:n]).to(state.device)
                dists  = poincare_distance_batch(state, stack)
                topk   = torch.topk(dists, min(self.K_CANDIDATES, n), largest=False)
                for i in topk.indices.tolist():
                    txt = tokens[i]
                    if not txt or len(txt) < self._MIN_TOKEN_LEN: continue
                    if not all(32 <= ord(ch) < 127 or ch.isalpha() for ch in txt): continue
                    cands.append((txt, vecs[i].to(state.device)))
                if cands:
                    return cands
...
def nearest_concept(soul):
    ...
    if li.size() > 0:
        results = li.tree.find_k_nearest(soul.state, k=20, max_depth=12)
        for dist, vec, meta in results:
...
def main():
    ...
    if args.init:
        ...
        for f in [args.soul, args.tree, args.buffer_file,
                  args.rg_vocab_file, args.rejected_file, lex_npz,
